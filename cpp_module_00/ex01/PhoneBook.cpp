...
void PhoneBook::displayContacts()
{
	...
	int contactCount = this->getAmount();
...
void PhoneBook::addContact(int *i)
{
	...
	while (1)
	{
		if (std::cin.eof())
			return;
		this->contacts[(*i)].setFirstName(edit_get_inp("Enter first name: "));
	...
	while (1)
	{
		if (std::cin.eof())
			return;
		this->contacts[(*i)].setLastName(edit_get_inp("Enter last name: "));
	...
	while (1)
	{
		if (std::cin.eof())
			return;
		this->contacts[(*i)].setNickname(edit_get_inp("Enter nickname: "));
	...
	while (1)
	{
		if (std::cin.eof())
			return;
		this->contacts[(*i)].setPhoneNumber(edit_get_inp("Enter phone number: "));
	...
	while (1)
	{
		if (std::cin.eof())
			return;
		this->contacts[(*i)].setDarkestSecret(edit_get_inp("Enter darkest secret: "));
...
int PhoneBook::searchPrintContact(std::string i_to_search)
{
	int index;
	int count;

	count = this->getAmount();
	if (count == 0)
	{
